...
use crate::{
    implementation::rocks::trie::RocksHashedCursorFactory, Account, HashedPostState,
    RocksTransaction,
};
...
fn test_rocks_cursor_comprehensive() {
    ...
    assert_eq!(current_value.nonce, values[0].nonce, "Current value after first() doesn't match");
    ...
    for i in 1..10 {
        ...
        assert_eq!(next_value.nonce, values[i].nonce, "Next value at index {} doesn't match", i);
    }
    ...
    assert_eq!(current_value.nonce, values[9].nonce, "Current value after last() doesn't match");
    ...
    for i in (0..9).rev() {
        ...
        assert_eq!(prev_value.nonce, values[i].nonce, "Prev value at index {} doesn't match", i);
    }
    ...
    for i in 0..10 {
        ...
        assert_eq!(seek_value.nonce, values[i].nonce, "Seek value at index {} doesn't match", i);
    }
...
fn test_rocks_cursor_single_entry() {
    ...
    let key = B256::from([1u8; 32]);
    let value =
        Account { nonce: 1, balance: U256::from(1000), bytecode_hash: Some(B256::from([1u8; 32])) };
    ...
    let (first_key, first_value) = first.unwrap();
    assert_eq!(first_key, key, "First key doesn't match expected value on single-entry database");
    assert_eq!(
...
fn test_rocks_hashed_account_cursor() {
    ...
    let account2 =
        Account { nonce: 2, balance: U256::from(2000), bytecode_hash: Some(B256::from([2; 32])) };
    let account3 =
        Account { nonce: 3, balance: U256::from(3000), bytecode_hash: Some(B256::from([3; 32])) };
